	...
	<div class="dropboxadmin">
		...
		<table class="ob-gridview" cellpadding="0" cellspacing="0">
			<tr>
				...
				<th><asp:LinkButton ID="LinkButton10" Text="Start&nbsp;Date" OnClick="sortStartDate" runat="server" /></th>
...
<asp:Repeater ID="rptContracts" runat="server" 
	...
	<ItemTemplate>
		<tr class='<%# Container.ItemIndex % 2 == 0 ? "" : "even" %>' valign="top">
			...
			<td class="contract"><%# Eval("ContractID")%></td>
			<td class="amount"><%# DataBinder.Eval(Container.DataItem, "Amount", "{0:C}")%></td>
			<td class="description"><%# DataBinder.Eval(Container.DataItem, "Comments") %></td>
		    <td class="contracttype"><%# DataBinder.Eval(Container.DataItem, "ContractType") %></td>
			<td class="startdate"><%# DataBinder.Eval(Container.DataItem, "StartDate", "{0:MM/dd/yyyy}")%></td>
			<td class="enddate"><%# DataBinder.Eval(Container.DataItem, "EndDate", "{0:MM/dd/yyyy}")%></td>
		</tr>
	</ItemTemplate>
</asp:Repeater>
